...
public class RoutineShoot extends SequentialCommandGroup {
  ...
  public RoutineShoot(Launcher launcher, SubCompressor compressor) {
    ...
    addCommands(new PreSpin(launcher, compressor).withTimeout(1.0), (new Shoot(launcher, compressor)).withTimeout(2.5)
        ,new RampMove(launcher, true).withTimeout(0.1), new RampMove(launcher, false).withTimeout(0.1));
  }
}
